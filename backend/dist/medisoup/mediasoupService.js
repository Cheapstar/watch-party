    ...
    constructor() {
        ...
        this.createWorker = async () => {
            try {
                ...
                newWorker.on("died", () => {
                    logger.error("Mediasoup worker died, exiting in 2 seconds.... [pid%id]", newWorker.pid);
        ...
        this.createRouter = async ({ roomId }) => {
            try {
                if (!this.worker) {
                ...
                logger.error(`Error while creating the router for the team with teamId:${roomId}`, error);
                throw error;
            }
        };
    }
